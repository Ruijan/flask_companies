...
def print_companies_to_html(df):
    ...
    keys = ["Name", "Sector", "Country", "Trailing P/E", "CAGR 3", "CAGR 5", "Payout ratio", "Div. Yield", "Continous Dividend Growth", "Div score"]
    ...
    while previous_index != -1:
        html = html[:previous_index] + "<tr onclick='showCompany(\"" + str(data.index.values[index])+ "\")'>" + html[previous_index+4:]
        index += 1
...
def get_corrected_div_values(dividends, stock_splits):
  ...
  for key in stock_splits.keys():
    ...
    if stock_splits[key] > 0:
        temp_dividends = {k: v/stock_splits[key] if datetime.strptime(k, "%b %d, %Y") < key_time else v for k, v in temp_dividends.items()}
...
def get_CAGR(yearly_dividends, years):
  ...
  if current_year - 1 in yearly_dividends.index and current_year - 1 - years in yearly_dividends.index:
    return (np.power(yearly_dividends.loc[current_year - 1] /yearly_dividends.loc[current_year - 1 - years] ,1/(years)) -1).values[0]
  elif current_year - 2 in yearly_dividends.index and current_year - 2 - years in yearly_dividends.index:
    return (np.power(yearly_dividends.loc[current_year - 2] /yearly_dividends.loc[current_year - 2 - years] ,1/(years)) -1).values[0]
  return 0
